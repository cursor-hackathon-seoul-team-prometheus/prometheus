...
interface HomeViewProps {
  ...
  onEditLecture: (lecture: SavedLecture) => void;
  onDeleteLecture: (id: string) => void;
}

const HomeView: React.FC<HomeViewProps> = ({ savedLectures, onStartNew, onSelectLecture, onEditLecture, onDeleteLecture }) => {
  return (
        ...
        {savedLectures.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedLectures.sort((a, b) => b.createdAt - a.createdAt).map((lecture) => {
              const hasSlides = lecture.htmlSlides && lecture.htmlSlides.length > 0;
              return (
                <div 
                  key={lecture.id} 
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 ${hasSlides ? 'bg-orange-100 text-orange-600 group-hover:bg-orange-600 group-hover:text-white' : 'bg-stone-100 text-stone-400 group-hover:bg-stone-800 group-hover:text-white'}`}>
                      {hasSlides ? (
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    {hasSlides && (
                      <span className="px-2.5 py-1 bg-orange-100 text-orange-600 rounded-lg text-[10px] font-bold uppercase tracking-wider">
                        슬라이드
                      </span>
                    )}
                  </div>
