  ...
  return (
    <div className="space-y-12">
      ...
      <section className="relative overflow-hidden bg-slate-900 rounded-[2.5rem] p-8 md:p-16 text-white shadow-2xl">
        <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 bg-indigo-500 rounded-full blur-[120px] opacity-20"></div>
        <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-60 h-60 bg-purple-500 rounded-full blur-[100px] opacity-10"></div>
        ...
        {savedLectures.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedLectures.sort((a, b) => b.createdAt - a.createdAt).map((lecture) => (
              <div 
                key={lecture.id} 
                className="bg-white rounded-[2rem] p-7 border border-slate-200/60 card-shadow card-hover transition-all group cursor-pointer flex flex-col min-h-[220px]"
              ...
              >
                ...
                <div className="flex-grow">
                  <h4 className="font-bold text-lg text-slate-800 line-clamp-2 leading-snug mb-3 group-hover:text-indigo-600 transition-colors">{lecture.title}</h4>
                </div>
                <div className="flex items-center text-xs font-bold text-slate-400 uppercase tracking-widest mt-4">
